...
class ResistanceModel:
    ...
    hill_2: float = 1.5
    # Optional THIRD drug, same scalar-or-(k,) convention as the second. Added for the tier iv-a
    # synthetic-lethal agent (`tier_iv_regimens`), where a third *concentration-gated* axis is
    # required rather than the `additional_kill` passthrough: routing a small molecule through
    # additional_kill would exempt it from the Emax saturation arithmetic that governs every other
    # small molecule here, and that arithmetic is precisely what demoted the CDK4/6 inhibitor from
    # "largest lever in the regimen" to a marginal one (`combination_regimen`.
    # REAL_POTENCY_CORRECTION_TO_THE_BAR). A mechanism must not be made to look better than the
    # CDK4/6 inhibitor by being modelled through a laxer channel.
    #
    # A scalar is right for a target every clone shares -- e.g. a synthetic-lethal vulnerability
    # conferred by a homozygous deletion, which every clone carries by descent because a deletion
    # cannot be un-deleted. A (k,) array specifies per-clone values, needed when the third agent's
    # target is droppable by some clones (the tier iv-b antibody/CAR case).
    ic50_nM_3: float | np.ndarray | None = None
    max_kill_3: float | np.ndarray | None = None
    hill_3: float = 1.5
    ...
    def __post_init__(self):
        ...
        for name in ("ic50_nM_2", "max_kill_2", "ic50_nM_3", "max_kill_3"):
...
# Extinction anchors in NORMALISED units (carrying_capacity = 1.0). One cell out of N at capacity.
# Neither is measured for canine HS; both are carried so the choice can be swept rather than
# asserted. See `validation.monotherapy_backtest` for why an extinction state is needed at all.
ONE_CELL_AT_1E9 = 1e-9
ONE_CELL_AT_1E12 = 1e-12
...
def simulate_resistance(model: ResistanceModel, concentration: np.ndarray, initial: np.ndarray,
                        ...
                        concentration_3: np.ndarray | None = None,
                        bystander_fraction: float = 0.0,
                        bystander_source_clone: int = 0,
                        extinction_threshold: float | None = None) -> np.ndarray:
    ...
    `extinction_threshold`, if given, zeroes any clone whose population falls below it, and a clone
    at zero stays there. WITHOUT it (the default, preserving every figure computed before it
    existed) the state is a pure continuous product, so a clone at 1e-82 -- vastly less than one
    cell -- is still a live population that regrows the moment drug pressure lifts. That is the
    defect `validation.monotherapy_backtest` diagnosed: it makes cure unrepresentable, which forces
    the engine to conclude that every finite course fails, whatever the drug or disease.

    The threshold belongs in NORMALISED units, where `carrying_capacity` is 1.0. One cell is
    therefore 1/N for a tumour of N cells at capacity -- roughly 1e-9 for 1e9 cells, 1e-12 for
    1e12. `ONE_CELL_*` below give the two defensible anchors; neither is measured for canine HS, so
    the choice is swept rather than asserted.

    It is left OFF by default deliberately. Turning it on changes published figures, and that
    change should be made by a caller that intends it, not silently inherited.
    """
    ...
    has_third_drug = concentration_3 is not None and model.ic50_nM_3 is not None
    for t, c in enumerate(np.asarray(concentration, float)):
        ...
        if has_third_drug:
            kill_3 = drug_kill_rate(concentration_3[t], model.ic50_nM_3, model.hill_3,
                                    model.max_kill_3)
            kill = kill + kill_3
            if bystander_fraction > 0:
                # ADC bystander effect: a membrane-permeable payload released when an
                # antigen-POSITIVE cell is killed also kills antigen-negative neighbours. The
                # local payload concentration any cell experiences scales with how much of its
                # neighbourhood is antigen-positive, so the term is proportional to the source
                # clone's population FRACTION -- which makes it self-limiting by construction:
                # as the antigen-positive population is destroyed, the bystander effect it
                # sustains decays with it. That self-limitation is the whole reason this cannot
                # be modelled as a flat `additional_kill` array.
                total = current.sum()
                if total > 0:
                    source_kill = np.atleast_1d(kill_3)[
                        bystander_source_clone if np.ndim(kill_3) else 0]
                    kill = kill + (bystander_fraction * source_kill
                                   * current[bystander_source_clone] / total)
        ...
        nxt = grown @ model.mutation
        if injections and (t + 1) in injections:
            nxt = nxt + injections[t + 1]
        if extinction_threshold is not None:
            # Applied AFTER mutation transfer and injection so a lineage that was just re-seeded
            # this day is not immediately wiped: extinction should act on what actually survives
            # the day, not on an intermediate.
            nxt = np.where(nxt < extinction_threshold, 0.0, nxt)
        state[t + 1] = nxt
    return state
...
def perturb_resistance_model(model: ResistanceModel, rng: np.random.Generator,
    ...
    if model.ic50_nM_3 is not None:
        updates["ic50_nM_3"] = model.ic50_nM_3 * rng.lognormal(0, ic50_scale)
...
def clone_growth_margins(model: ResistanceModel, concentration: float,
                         ...
                         concentration_3: float | None = None) -> np.ndarray:
    ...
    if concentration_3 is not None and model.ic50_nM_3 is not None:
        kill = kill + drug_kill_rate(concentration_3, model.ic50_nM_3, model.hill_3, model.max_kill_3)
...
def run_monte_carlo_two_compartment(reference: ResistanceModel, css_reference: float, horizon_days: int,
                                    ...
                                    css_reference_2_duration_days: int | None = None,
                                    css_reference_3: float | None = None,
                                    exposure_scale_3: float = .3,
                                    bystander_fraction: float = 0.0,
    ...
    `additional_kill` is an optional precomputed `(horizon_days, k)` per-clone kill-rate array added
    on top of everything else, including the vaccine term when both are active. It exists for a third
    mechanism whose selectivity is neither concentration-gated (the two drugs) nor keyed to the
    vaccine's antigen mask -- an oncolytic virus being the motivating case, since its permissiveness
    is set by Ras-pathway activation rather than by MHC presentation, so its clone coverage genuinely
    differs from the vaccine's (see `alternative_modalities`). When both are active the sum is used
    for the provisional antigen-positive-cell-days run too, so immune-escape seeding reflects the
    burden actually present under the full regimen rather than under the vaccine alone.

    Note that `additional_kill` drives no escape-clone seeding of its own: the immune_escape clone is
    an MHC-I/antigen-loss event selected by the vaccine, and a virus-resistance route (loss of entry
    receptor, restored type-I-IFN response) is a mechanistically different escape this state vector
    does not represent. Modeling a virus without that route is an optimistic omission, not a neutral
    one -- see `alternative_modalities.UNMODELED_ESCAPE_ROUTES`.

    All vaccine parameters default to off (`vaccine_start_day=None`, `vaccine_max_kill=0.0`) and
    `additional_kill` defaults to None, so every existing caller keeps byte-identical behavior
    including RNG draw order.
    """
    rng = np.random.default_rng(seed)
    k = len(reference.growth)
    # Fail here, not stochastically mid-run. `_dominant_mechanism` indexes `clone_names` by the
    # winning clone, so a model with more clones than names only raises IndexError on the trials
    # where one of the unnamed clones happens to dominate -- a 5-clone model can therefore run
    # clean for 100 trials and crash on the 101st, or worse, run clean and be believed.
    if len(clone_names) != k:
        raise ValueError(f"clone_names has {len(clone_names)} entries but the model has {k} "
                         f"clones; pass names for every clone (the default covers only the "
                         f"{len(CLONE_NAMES)} base clones)")
    identity_model = replace(reference, mutation=np.eye(k))
    detection_floor = detection_floor_fraction * reference.carrying_capacity
    ...
    if additional_kill is not None:
        additional_kill = np.asarray(additional_kill, dtype=float)
        if additional_kill.shape != (horizon_days, k):
            raise ValueError(f"additional_kill must have shape (horizon_days, k) = "
                             f"({horizon_days}, {k}); got {additional_kill.shape}")
        extra_kill = additional_kill if vaccine_kill is None else vaccine_kill + additional_kill
    else:
        extra_kill = vaccine_kill
    ...
    for trial in range(trials):
        ...
        if css_reference_3 is not None:
            concentration_3 = np.full(horizon_days,
                                      css_reference_3 * rng.lognormal(0, exposure_scale_3))
        ...
        for initial in (primary_initial, nodal_initial):
            ...
            sensitive_trajectory = simulate_resistance(model, concentration, sensitive_only,
                                                        ...
                                                        concentration_3=concentration_3,
                                                        bystander_fraction=bystander_fraction)[:, 0]
            jittered_rates = seeding_rates * rng.lognormal(0, seeding_rate_scale, len(seeding_rates))
            ...
            if vaccine_active:
                ...
                provisional = simulate_resistance(model, concentration, initial, injections,
                                                  concentration_2, additional_kill=extra_kill,
                                                  concentration_3=concentration_3,
