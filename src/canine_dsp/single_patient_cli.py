...
def single_patient_demo(out: Path, breed: str = "bmd",
    ...
    'golden_retriever') governs findings 1-4 only -- see finding 5 for why a patient with this presentation is
    handled as a separate scenario rather than a value of this parameter.
      ...
      5. THE UNSEQUENCED-BREED CASE -- findings 1-4 are BMD-context end to end (driver frequency, tumor
         scenario, calibration benchmark). Reusing them for a patient with this presentation would repeat the same
         species/lineage-mismatch error this session already caught with canine-melanoma CDK4/6i
         potency data standing in for canine-HS potency. This runs the actual pulmonary
         two-compartment scenario against the actual breed-matched benchmark instead.
    """
    if breed not in ("bmd", "flat_coated_retriever"):
        raise ValueError(
            ...
            "patient with this presentation is NOT covered by this arm at all: see finding_5_this_is_a_different_"
            "disease_not_a_different_breed_name below, which runs the actual pulmonary "
            "two-compartment scenario against the actual breed-matched benchmark instead of "
            "reusing this arm's numbers.")
    out.mkdir(parents=True, exist_ok=True)

    # 1. Driver conditioning ------------------------------------------------------------------
    driver = driver_conditioned_arms(breed=breed)
    driver_unsequenced_breed = driver_conditioned_arms(breed="unsequenced")
    ...
    # 5. The actual case: a different scenario against a different, breed-matched benchmark --
    ...
    # objection already caught once this session -- so this runs the real breed-specific scenario
    # (`pulmonary_hs_scenarios`, the two-compartment nodal model) instead of reusing the BMD arm.
    site_rows = []
    for nodal_prob in NODAL_INVOLVEMENT_PROB_SWEEP:
        model, site_css, site_rates, site_debulk, _ = pulmonary_hs_scenarios(
            cdk46_max_kill=0.0, debulking_fraction=debulking_fraction,
            nodal_involvement_prob_values=[nodal_prob])[nodal_prob]
        site_outcome = run_monte_carlo_two_compartment(
            model, site_css, horizon_days, site_rates, nodal_involvement_prob=nodal_prob,
            nodal_seed_fraction=NODAL_SEED_FRACTION, debulking_fraction=site_debulk, trials=trials,
            preexisting_prob=preexisting_prob, initial_burden=_PULMONARY_BASELINE_BURDEN, seed=seed)
        site_partition = partition_two_compartment_by_preexisting_subclone(site_outcome)
        site_rows.append({
            "nodal_involvement_prob": nodal_prob,
            "probability_durable_response": float(1 - site_outcome.progressed.mean()),
            "durable_without_preexisting_subclone":
                site_partition["without_preexisting_subclone"]["durable_response_fraction"],
            "durable_with_preexisting_subclone":
                site_partition["with_preexisting_subclone"]["durable_response_fraction"],
            "median_days_to_progression": (
                float(np.nanmedian(site_outcome.time_to_progression))
                if site_outcome.progressed.any() else None),
        })
    site_df = pd.DataFrame(site_rows)
    site_df.to_csv(out / "pulmonary_arm.csv", index=False)
    ...
    summary = {
        ...
        "answer": "Yes to both, and the two findings point in opposite directions. Sequencing helps "
                 ...
                 "input here comes from. Finding 5 checks whether it transfers to a patient with this presentation "
                 "and finds it does not: this HS presentation is a separately-published, worse-prognosis, "
        ...
        "finding_1_driver_conditioning_dominates": {
            ...
            **driver,
            ...
            "does_not_apply_to_other_breeds": "This finding is specific to the breed named in 'for_breed' "
                "above. See finding_5 for what driver conditioning looks like for an unsequenced breed, which is "
        ...
        "finding_5_this_is_a_different_disease_not_a_different_breed_name": {
            "why_this_section_exists": "Findings 1-4 model BMD localized PIHS end to end -- the "
                ...
                "this HS presentation is a clinically distinct, separately-published presentation (Sakai et al. "
                ...
                "for a patient with this presentation would be the same category of mistake this session already "
                ...
                "equivalent boundary.",
            "driver_conditioning_for_this_case": driver_unsequenced_breed,
            "scenario_used": "pulmonary_hs_scenarios / run_monte_carlo_two_compartment -- the "
            ...
            "cdk46_or_ccnu_not_added_here": "Deliberately not layered onto this arm: doing so would "
                ...
                "transfer to the pulmonary tumor model, which is exactly the unverified cross-lineage "
                "step this finding exists to avoid taking silently.",
            "matched_benchmark": BREED_MATCHED_PULMONARY_HS_BENCHMARK,
            ...
            "verdict": "The honest N=1 statement for a patient with this presentation is not 'apply findings 1-4 with "
                "a discount' -- it is that this repo's own best-evidenced breed-specific model "
                ...
                "duration cap) has been checked against breed-specific growth rates or potency data, "
                "because none exist.",
        },
        ...
        "unverified_extrapolations": [
            ...
            "the pulmonary arm's own scenario (pulmonary_hs_scenarios) still reuses the BMD growth-"
            "rate/resistance-mechanism spectrum unchanged, per that function's own documented "
            "reasoning: no presentation-specific germline locus or driver panel is established enough to "
